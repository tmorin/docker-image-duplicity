...
image_versions=$(docker-tags ${tag_image} | jq --raw-output '.["tags"] | to_entries | .[] | .value' | grep "${duplicity_version}-")
echo "image_versions: $image_versions"

duplicity_version_major=$(echo ${duplicity_version} | sed -E "s/([0-9]*)\.[0-9]*\.[0-9]*/\1/")
duplicity_version_minor=$(echo ${duplicity_version} | sed -E "s/([0-9]*\.[0-9]*)\.[0-9]*/\1/")

tag_versions="${duplicity_version} ${duplicity_version_major} ${duplicity_version_minor}"
